...
class Data:
    ...
    def __init__(self, filename=None):
        ...
        self.header2ind = {}
        self.header2ind_num = {}
        self.numeric_matrix = numpy.matrix([]) # hold numeric data matrix
        if filename!=None:
            self.read(filename)
    ...
    def read(self, filename):
        ...
        # header2ind dictionary
        i = 0
        for h in self.headers:
            self.header2ind.update({ h : i })
            i = i+1
        
        # header2ind_num dictionary
        i = 0
        for h in self.numeric:
            self.header2ind_num.update( {h : i} )
            i = i+1
...
class PCAData(Data):
    ...
    def __init__(self, pdata, ei_vec, ei_value, mean, headers):
        Data.__init__(self)
        self.eigenvalues = ei_value
        self.eigenvectors = ei_vec
        self.mean = mean
        self.ori_headers = headers

        self.numeric_matrix = pdata  # field from parent class
        self.dimensions = self.numeric_matrix.shape
        for row in pdata:
            list = []
            for col in row:
                list.append(col[0].tolist()[0])
            self.NumPy.append(list[0]) # raw data
        

        for i in range( len(self.ori_headers) ):
            self.types.append("numeric")
            new_h = "PCA" + str(i)
            self.headers.append(new_h)
            self.numeric.append(new_h)
            self.header2col.update({new_h:"numeric"})
            self.header2ind.update({new_h:i})
            self.header2ind_num.update({new_h:i})
